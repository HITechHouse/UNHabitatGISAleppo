    ...
    if (layer && map) {
      if (isVisible) {
        if (!map.hasLayer(layer)) {
          // إضافة الطبقة إلى الخريطة أو إلى المجموعة الرئيسية
          if (window.mainLayersGroup && (layerName === "neighborhoods-labels" || layerName === "service-sectors-labels")) {
            // إضافة إلى المجموعة الرئيسية إذا كانت موجودة
            if (!window.mainLayersGroup.hasLayer(layer)) {
              window.mainLayersGroup.addLayer(layer);
              console.log("Added layer to main group:", layerName);
            }
          } else {
            // إضافة مباشرة إلى الخريطة
            map.addLayer(layer);
        ...
        // أيضاً إزالة من المجموعة الرئيسية إذا كانت موجودة
        if (window.mainLayersGroup && window.mainLayersGroup.hasLayer(layer)) {
          window.mainLayersGroup.removeLayer(layer);
          console.log("Removed layer from main group:", layerName);
        }
      }
    } else {
