...
function initStyleControls() {
  ...
  const layerLineWeight = document.getElementById("layerLineWeight");
  ...
  // show the line weight value
  if (layerLineWeight && lineWeightValue) {
    layerLineWeight.addEventListener("input", function () {
  ...
  if (applyStyleBtn) {
    applyStyleBtn.addEventListener("click", function () {
      if (!layerStyleSelect || !layerColor || !layerOpacity || !layerLineWeight || !layerLineColor)
        return;

      const layer = layerStyleSelect.value;
      const color = layerColor.value;
      const opacity = parseFloat(layerOpacity.value);
      const weight = parseInt(layerLineWeight.value);
      const lineColor = layerLineColor.value;
      ...
      console.log("Applying style with values:", { layer, color, opacity, weight, lineColor });
      
      // Add a small delay to ensure the style is applied properly
      setTimeout(() => {
        applyLayerStyle(layer, { color, opacity, weight, lineColor });
      }, 100);
    });
  }
  ...
  if (resetStyleBtn) {
    resetStyleBtn.addEventListener("click", function () {
      ...
      if (
        ...
        if (layerColor) layerColor.value = "#1e40af";
        if (layerOpacity) {
          layerOpacity.value = "0.8";
          if (opacityValue) opacityValue.textContent = "0.8";
        }
        if (layerLineWeight) {
          layerLineWeight.value = "2";
          if (lineWeightValue) lineWeightValue.textContent = "2";
        }
        if (layerLineColor) layerLineColor.value = "#000000";
        if (layerStyleSelect) layerStyleSelect.value = "";

        // Reset all layers to default
        if (window.resetAllLayersToDefault) {
          window.resetAllLayersToDefault();
        }

        // Show success notification
        if (window.showSuccessNotification) {
          window.showSuccessNotification("تم إعادة ضبط جميع الأنماط بنجاح!");
        }
      }
    });
  }
...
function applyLayerStyle(layerName, style) {
  ...
  if (window.applyMapLayerStyle) {
    console.log("Calling applyMapLayerStyle with:", layerName, style);
