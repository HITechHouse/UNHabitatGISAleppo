...
async function generateNeighborhoodReport(neighborhoodId, neighborhoodName) {
  try {
    ...
    const neighborhoodFeature = neighborhoodsData.features.find(
      (feature) =>
        feature.properties.ID.toString() === neighborhoodId.toString()
    );
    ...
    console.log(
      "صورة الخريطة:",
      mapImageData ? "تم التقاطها بنجاح" : "فشل في التقاط الصورة"
    );
    ...
    const reportHtml = `
      ...
      <div class="report-page">
        <div class="report-header" style="text-align: center; margin-bottom: 30px; border-bottom: 3px solid #2196F3; padding-bottom: 20px;">
          ...
          <div class="report-subtitle" style="color: #666; font-size: 18pt; margin-bottom: 10px;">${convertToEnglishNumbers(
            neighborhoodName
          )}</div>
          <div class="report-date" style="color: #888; font-size: 11pt;">تاريخ الإنشاء: ${convertToEnglishNumbers(
            new Date().toLocaleDateString("en-US")
          )}</div>
          <div class="report-date" style="color: #888; font-size: 11pt;">رقم التقرير: ${convertToEnglishNumbers(
            neighborhoodId
          )}-${convertToEnglishNumbers(Date.now().toString().slice(-6))}</div>
        </div>
          ...
          <div class="map-container">
            ${
              ...
              <div class="coordinates-box">
                ...
                خط الطول: ${convertToEnglishNumbers(
                  neighborhoodFeature.geometry.coordinates[0][0][0].toFixed(6)
                )}<br>
                خط العرض: ${convertToEnglishNumbers(
                  neighborhoodFeature.geometry.coordinates[0][0][1].toFixed(6)
                )}
              </div>
            `
                : `
        ...
        <div class="section" style="margin-bottom: 200px;">
          ...
          `
            ...
            <div style="text-align: center; padding: 20px; background: #fff3cd; border: 1px solid #ffeaa7; border-radius: 5px; color: #856404;">
              ...
              <small>البيانات الأساسية: معرف الحي ${convertToEnglishNumbers(
                neighborhoodId
              )}</small>
            </div>
          `
        ...
        <div class="section" style="margin-bottom: 300px;">
          ...
          ${
            ...
            <table>
              ...
              ${sectoralFunctionsData
                .map(
                  (sector) => `
                <tr>
                  <td style="font-weight: bold;">${convertToEnglishNumbers(
                    sector.name
                  )}</td>
                  <td style="text-align: center; font-weight: bold; color: ${getSectorColor(
                    sector.percentage
                  )};">${convertToEnglishNumbers(
                    sector.percentage.toFixed(1)
                  )}%</td>
                  <td style="text-align: center;" class="${getStatusClass(
                    sector.status
                  )}">${convertToEnglishNumbers(sector.status)}</td>
                  <td style="text-align: center;">${convertToEnglishNumbers(
                    sector.assessment
                  )}</td>
                </tr>
              `
            ...
            <div style="margin-top: 20px; padding: 15px; background: #e8f5e8; border: 1px solid #4CAF50; border-radius: 5px;">
              ...
              <div style="margin-top: 10px;">
                <div>• عدد الوظائف المقيمة: <strong>${convertToEnglishNumbers(
                  sectoralFunctionsData.length
                )}</strong></div>
                <div>• متوسط الأداء العام: <strong style="color: ${getSectorColor(
                  sectoralFunctionsData.reduce(
        ...
        <div class="section">
          ...
          <table>
            ...
            ${damageRatiosData
              ...
              <tr>
                ...
                <td style="text-align: center; font-weight: bold;">${convertToEnglishNumbers(
                  damage.percentage
                )}${typeof damage.percentage === "number" ? "%" : ""}</td>
                <td style="text-align: center;">
                  <span class="damage-indicator damage-${damage.level.toLowerCase()}">${convertToEnglishNumbers(
                  damage.levelText
                )}</span>
                </td>
                <td style="text-align: center; color: ${
                  damage.priorityColor
                }; font-weight: bold;">${convertToEnglishNumbers(
                  damage.priority
                )}</td>
              </tr>
            `
          ...
          ${
            ...
            <div style="margin-top: 20px; padding: 15px; background: #e3f2fd; border: 1px solid #2196F3; border-radius: 5px;">
              ...
              <div style="margin-top: 10px;">
                <div>• إجمالي نسبة الأضرار الشديدة: <strong style="color: #F44336;">${convertToEnglishNumbers(
                  damageRatiosData
                    .filter(
                      (d) =>
                        d.level === "HIGH" && typeof d.percentage === "number"
                    )
                    .reduce((sum, d) => sum + d.percentage, 0)
                    .toFixed(1)
                )}%</strong></div>
                <div>• إجمالي نسبة الأضرار المتوسطة: <strong style="color: #FF9800;">${convertToEnglishNumbers(
                  damageRatiosData
                    .filter(
                      (d) =>
                        d.level === "MEDIUM" && typeof d.percentage === "number"
                    )
                    .reduce((sum, d) => sum + d.percentage, 0)
                    .toFixed(1)
                )}%</strong></div>
                <div>• إجمالي نسبة الأضرار القليلة: <strong style="color: #4CAF50;">${convertToEnglishNumbers(
                  damageRatiosData
                    .filter(
                      (d) =>
                        d.level === "LOW" && typeof d.percentage === "number"
                    )
                    .reduce((sum, d) => sum + d.percentage, 0)
                    .toFixed(1)
                )}%</strong></div>
        ...
        <div class="report-footer" style="margin-top: 40px; text-align: center; color: #666; font-size: 10pt; border-top: 2px solid #e0e0e0; padding-top: 20px;">
          ...
          )} - جميع الحقوق محفوظة | تقرير رقم: ${convertToEnglishNumbers(
      neighborhoodId
    )}-${convertToEnglishNumbers(Date.now().toString().slice(-6))}</p>
        </div>
      </div>
    `;
    ...
    if (canvas) {
      ...
      const filename = `تقرير_شامل_${neighborhoodName.replace(
        /\s+/g,
        "_"
      )}_${timestamp}.pdf`;
      pdf.save(filename);
    ...
    if (typeof showNotification === "function") {
      showNotification(
        "تم تصدير التقرير الشامل بنجاح! تحقق من مجلد التنزيلات",
        "success"
      );
    }
  } catch (error) {
...
function getNeighborhoodDetailedInfo(neighborhoodFeature) {
  ...
  const info = [
    {
      ...
      value: convertToEnglishNumbers(
        properties.Names || properties.Name || properties.name || "غير محدد"
      ),
    },
    {
      label: "معرف الحي",
      value: convertToEnglishNumbers(properties.ID || "غير محدد"),
    },
    {
      label: "القطاع الإداري",
      value: convertToEnglishNumbers(
        properties.Sector || properties.SECTOR || "غير محدد"
      ),
    },
    {
      label: "المساحة التقديرية",
      value: `${convertToEnglishNumbers(areaInHectares)} هكتار`,
    },
    {
      label: "عدد السكان المقدر",
      value: convertToEnglishNumbers(
        properties.Population || properties.POPULATION || "غير محدد"
      ),
    },
    {
      label: "الكثافة السكانية",
      value: convertToEnglishNumbers(
        properties.Density || properties.DENSITY || "غير محدد"
      ),
    },
    {
      label: "نوع المنطقة",
      value: convertToEnglishNumbers(
        properties.Type || properties.ZONE_TYPE || properties.Zone || "غير محدد"
      ),
    },
    {
      label: "حالة التطوير",
      value: convertToEnglishNumbers(
        properties.Development || properties.STATUS || "غير محدد"
      ),
    },
    {
      label: "الرمز البريدي",
      value: convertToEnglishNumbers(
        properties.PostalCode || properties.POSTAL_CODE || "غير محدد"
      ),
    },
    {
      label: "آخر تحديث",
      value: convertToEnglishNumbers(new Date().toLocaleDateString("en-US")),
    },
  ];
  ...
  return info.filter(
    (item) =>
      item.value && item.value !== "غير محدد" && item.value !== "undefined"
  );
}
  ...
  if (typeof getDamageRatiosForNeighborhood === "function") {
    ...
    if (damageInfo) {
      damageData = [
        ...
        {
          type: "أضرار مرافق البنية التحتية",
