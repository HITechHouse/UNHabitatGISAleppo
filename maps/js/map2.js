...
function updateMapInfoBar() {
  ...
  if (scaleElement && t && t.mapInfo) {
    ...
    scaleElement.textContent = `${t.mapInfo.scale}: 1:${scale.toLocaleString(
      "en-US"
    )}`;
  }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (layersBtn && basemapGallery) {
    ...
    document.addEventListener("click", function (e) {
      if (!basemapGallery.contains(e.target) && e.target !== newLayersBtn) {
        basemapGallery.classList.remove("open");
      }
    });
  ...
  var tryPatch = function () {
    if (
      ...
      window.renderInfoPanel = function (tabId, neighborhoodId) {
        ...
        if (
          tabs &&
          (tabs.classList.contains("hidden") || tabs.style.display === "none")
        ) {
...
function getDamageRatiosForNeighborhood(neighborhoodId) {
  console.log("Getting damage ratios for neighborhood ID:", neighborhoodId);
    ...
    if (props.severeDamagePercentage || props.damage_level) {
      return {
        severeDamagePercentage:
          props.severeDamagePercentage || (props.damage_level > 75 ? 80 : 0),
        mediumDamagePercentage:
          props.mediumDamagePercentage || (props.damage_level > 50 ? 60 : 0),
        lightDamagePercentage:
          props.lightDamagePercentage || (props.damage_level > 25 ? 40 : 0),
        noDamagePercentage:
    ...
    if (window.map && window.map.eachLayer) {
      window.map.eachLayer(function (layer) {
        ...
        if (
          ...
          layer.feature &&
          layer.feature.properties &&
          (layer.feature.properties.Name || layer.feature.properties.OBJECTID)
        ) {
          shouldReset = true;
        }
...
// Note: Style controls are now handled in sidebar.js to avoid conflicts
// This code has been moved to sidebar.js for better organization

// apply the styles to the neighborhoods layer
function applyStyleToNeighborhoods(color, opacity, lineWeight, lineColor) {
  try {
    let layersStyled = 0;
    const styleObject = {
      color: lineColor || color, // Use lineColor if provided, otherwise use color
      fillColor: color,
      fillOpacity: opacity,
    ...
    if (layersStyled === 0) {
      console.warn("لم يتم العثور على أي طبقة من الأحياء لتطبيق الأنماط عليها");
    }
  } catch (error) {
...
function applyStyleToServiceSectors(color, opacity, lineWeight, lineColor) {
  try {
    let layersStyled = 0;
    const styleObject = {
      color: lineColor || color, // Use lineColor if provided, otherwise use color
    ...
    console.log(`تم تطبيق الأنماط على ${layersStyled} طبقة من دوائر الخدمات`);
...
// Global function to apply styles to any layer
window.applyMapLayerStyle = function (layerName, style) {
  try {
    const { color, opacity, weight, lineColor } = style;
    
    console.log("Applying style to layer:", layerName, "with style:", style);
    ...
    console.log("Final style object:", {
      lineColor: lineColor,
      fillColor: color,
      borderColor: styleObject.color,
      weight: styleObject.weight
    });
    
    console.log("Style object:", styleObject);
    
    // Prevent multiple style applications
    if (window.currentStyleApplication && window.currentStyleApplication.layer === layerName) {
      console.log("Preventing duplicate style application for layer:", layerName);
      return;
    }
    
    window.currentStyleApplication = { layer: layerName, timestamp: Date.now() };

    let layersStyled = 0;

    // Apply styles based on layer name
    if (layerName === "neighborhoods" || layerName === "الأحياء") {
      // Apply to neighborhoods layer
      if (window.map && window.map.eachLayer) {
        window.map.eachLayer(function (layer) {
          if (
            layer.feature &&
      ...
      // Try to access through global variables
      const possibleVariables = [
        "neighborhoodsLayer",
        "neighborhoods",
        "neighborhoodsData",
    ...
    } else if (layerName === "service-sectors" || layerName === "دوائر الخدمات") {
      // Apply to service sectors layer
      if (window.map && window.map.eachLayer) {
        window.map.eachLayer(function (layer) {
          if (
            layer.feature &&
    ...
    console.log(`تم تطبيق الأنماط على ${layersStyled} طبقة من ${layerName}`);

    if (layersStyled === 0) {
      console.warn(`لم يتم العثور على أي طبقة من ${layerName} لتطبيق الأنماط عليها`);
    }

    // Show success notification
    showSuccessNotification(`تم تطبيق الأنماط على طبقة ${layerName} بنجاح!`);
    
    // Clear the current style application after a delay
    setTimeout(() => {
      if (window.currentStyleApplication && window.currentStyleApplication.layer === layerName) {
        delete window.currentStyleApplication;
      }
    }, 1000);
