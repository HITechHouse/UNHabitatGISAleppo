...
window.addEventListener("DOMContentLoaded", function () {
  ...
  // Stop drawing tools when clicking on map outside of drawing area
  map.on("click", function (e) {
    // If we're in drawing mode and click outside the drawing area, stop drawing
    if (activeDrawShape && !e.originalEvent.target.classList.contains("leaflet-draw-tooltip")) {
      // Don't stop immediately, let the user complete their drawing
      // This is just for reference - the draw:drawstop event will handle the actual stopping
    }
  });

  // Stop drawing tools on double click
  map.on("dblclick", function (e) {
    // Prevent default double click behavior (zoom) when drawing tools are active
    if (activeDrawShape || activeDraw) {
      ...
      if (activeDrawShape) {
        activeDrawShape.disable();
        activeDrawShape = null;
        console.log("Drawing tool stopped by double click");
      }
      if (activeDraw) {
        activeDraw.disable();
        activeDraw = null;
        console.log("Measurement tool stopped by double click");
      }
    }
  });
  ...
  document.addEventListener("keydown", function (e) {
    ...
    // Stop drawing tools when Escape is pressed
    if (e.key === "Escape") {
      if (activeDrawShape) {
        activeDrawShape.disable();
        activeDrawShape = null;
  ...
  // Draw stop event to disable drawing tool after completion
  ...
  // Draw vertex event to handle drawing completion
  map.on("draw:drawvertex", function (e) {
    // This event fires when a vertex is added during drawing
    // We can use this to detect when drawing is complete for certain shapes
  });

  // Draw start event to track when drawing begins
  map.on("draw:drawstart", function (e) {
    // This event fires when drawing starts
  ...
  function showMeasureResultBox(result, type) {
    ...
    if (typeof result === 'string') {
      ...
      displayResult = `${typeText}: ${result.toFixed(2)} ${unit}`;
    } else {
      displayResult = "نتيجة غير صحيحة";
    }
  ...
  if (applyStyleBtn) {
    applyStyleBtn.addEventListener("click", function () {
      ...
      const layerLineColor = document.getElementById("layerLineColor");
      ...
      setTimeout(() => {
        const style = {
          ...
          lineColor: layerLineColor ? layerLineColor.value : "#000000",
        };
 ...
 */
function initMap() {
  // define the home view and zoom level
  const homeView = [36.2021, 37.1343];
  const homeZoom = 12;
  ...
  map = L.map("map", {
    center: homeView,
    zoom: homeZoom,
    preferCanvas: true, // Use canvas renderer for better performance and export
  });
  ...
  L.Icon.Default.mergeOptions({
    ...
    iconUrl: 'js/lib/images/marker-icon.png',
  ...
  map.on("draw:created", function (e) {
    ...
    if (e.layerType === "polyline") {
      ...
      // For markers, just show coordinates
      const latlng = layer.getLatLng();
      result = `الإحداثيات: ${latlng.lat.toFixed(6)}, ${latlng.lng.toFixed(6)}`;
    }
    ...
    // Show result in floating box (only if result is not empty)
    if (result && result.trim()) {
      showMeasureResultBox(result, e.layerType);
    }
  });
...
function loadNeighborhoodsLayer() {
  ...
  const validNeighborhoods = neighborhoodsData.features.filter((feature) => {
    ...
    if (!feature.geometry || !feature.geometry.coordinates) {
      console.warn(
        "Neighborhood feature without geometry:",
        feature.properties
      );
      return false;
    }
      ...
      for (let i = 0; i < coords.length; i++) {
        ...
        for (let j = 0; j < ring.length; j++) {
          ...
          if (
            ...
            console.warn(
              "Invalid coordinate in neighborhood:",
              coord,
              "in feature:",
              feature.properties
            );
  ...
  console.log(
    `Valid neighborhood features: ${validNeighborhoods.length} out of ${neighborhoodsData.features.length}`
  );
    ...
    try {
      ...
      if (validCoords > 0) {
        ...
        if (isNaN(centerLat) || isNaN(centerLng)) {
          console.warn(
            "Invalid center coordinates calculated for feature:",
            feature.properties
          );
          return;
        }
        ...
        console.warn(
          "No valid coordinates found for feature:",
          feature.properties
        );
      }
    } catch (e) {
      console.error(
        "Error processing neighborhood label:",
        e,
        feature.properties
      );
    }
  ...
  if (mainLayersGroup) {
    ...
    // لا تضيف طبقة مسميات الأحياء بشكل افتراضي
  ...
  try {
    if (
      neighborhoodsLayer &&
      neighborhoodsLayer.getLayers &&
      neighborhoodsLayer.getLayers().length > 0
    ) {
...
function loadAleppoCityLayer() {
  ...
  if (
    !aleppoCityGeoJSON ||
    !aleppoCityGeoJSON.features ||
    !Array.isArray(aleppoCityGeoJSON.features)
  ) {
  ...
  const validFeatures = aleppoCityGeoJSON.features
    .map((feature, index) => {
      // Check if feature has valid geometry
      if (!feature.geometry || !feature.geometry.coordinates) {
      ...
      if (feature.geometry.type === "MultiPolygon") {
        ...
        if (
          ...
          console.log(
            "Fixing MultiPolygon structure: converting flat coordinate array to proper MultiPolygon"
          );
            ...
            for (let j = 0; j < coords[i].length && allValid; j++) {
              ...
              for (let k = 0; k < ring.length && allValid; k++) {
                ...
                if (
                  ...
                  console.warn(
                    "Invalid coordinate in MultiPolygon:",
                    coord,
                    "at position",
                    i,
                    j,
  ...
  console.log(
    `Valid Aleppo city features: ${validFeatures.length} out of ${aleppoCityGeoJSON.features.length}`
  );
...
/**
 * ترجمة حالة مرافق البنية التحتية
...
function loadServiceSectorsLayer() {
  ...
  if (mainLayersGroup) {
    ...
    // لا تضيف طبقة مسميات دوائر الخدمات بشكل افتراضي
...
function createAleppoCityPopup(feature, layer) {
  ...
  const area = properties.Shape_Area
    ? (properties.Shape_Area / 1000000).toFixed(2)
    : "غير متوفر"; // Convert to km²
  const perimeter = properties.Shape_Length
    ? (properties.Shape_Length / 1000).toFixed(2)
    : "غير متوفر"; // Convert to km
...
function showCityOverview() {
  ...
  if (window.setSelectedCity) {
    ...
    const totalNeighborhoods = neighborhoodsData
      ? neighborhoodsData.features.length
      : 0;
    const totalServiceSectors = window.serviceSectorsData
      ? window.serviceSectorsData.features.length
      : 0;
