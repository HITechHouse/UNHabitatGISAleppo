...
function applyStyleToNeighborhoods(color, opacity, lineWeight, lineColor) {
  try {
    console.log(`تطبيق الأنماط على الأحياء:`, { color, opacity, lineWeight, lineColor });
    let layersStyled = 0;
    const styleObject = {
...
function applyStyleToServiceSectors(color, opacity, lineWeight, lineColor) {
  try {
    console.log(`تطبيق الأنماط على دوائر الخدمات:`, { color, opacity, lineWeight, lineColor });
    let layersStyled = 0;
    const styleObject = {
      color: lineColor || color,
      fillColor: color,
...
function applyStyleToAleppoCity(color, opacity, lineWeight, lineColor) {
  try {
    console.log(`تطبيق الأنماط على مدينة حلب:`, { color, opacity, lineWeight, lineColor });
    let layersStyled = 0;
    const styleObject = {
      color: lineColor || color,
      fillColor: color,
...
window.applyMapLayerStyle = function (layerName, style) {
  ...
  const lineColor = style.lineColor || "#000000";

  try {
    switch (layerName) {
      case "aleppo-city":
        applyStyleToAleppoCity(color, opacity, weight, lineColor);
        showSuccessNotification("تم تطبيق الأنماط على طبقة مدينة حلب بنجاح!");
        break;

      case "neighborhoods":
        applyStyleToNeighborhoods(color, opacity, weight, lineColor);
        showSuccessNotification("تم تطبيق الأنماط على طبقة الأحياء بنجاح!");
        break;

      case "service-sectors":
        applyStyleToServiceSectors(color, opacity, weight, lineColor);
        showSuccessNotification(
          "تم تطبيق الأنماط على طبقة دوائر الخدمات بنجاح!"
        );
        break;
