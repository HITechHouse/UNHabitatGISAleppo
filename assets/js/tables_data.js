...
window.translations = {
  ar: {
    ...
    infrastructureStatuses: "مستوى الأضرار على الأبنية",
    ...
    transportationMobilityTypes: "أقسام النقل والتنقل",
    mobilityCentersTypes: "مرافق النقل",
    suppliesNames: "أنواع المؤون والبضائع",
    ...
    availabilityStatuses: "واقع الكادر التشغيلي / المستهلكات",
    infrastructureFacilities: "مرافق البنية التحتية",
    administrativeServicesNames: "أنواع الخدمات الإدارية",
    otherServicesNames: "الخدمات الإدارية",
    facilityTypes: "أنواع المنشآت",
    participationTypes: "أنواع المشاركة",
    potentialPartners: "الشركاء المحتملين للوحدات الإدارية",
    marketTypes: "أنواع الأسواق",
    randomLocations: "المواقع العشوائية",
    streetCleanlinessLevels: "مستويات نظافة الشوارع",
    insectControls: "أنشطة مكافحة الحشرات والقوارض",
  ...
  en: {
    ...
    infrastructureStatuses: "Building Damage Levels",
    ...
    humanNeedsTypes: "Humanitarian Needs Types",
    ...
    transportationMobilityTypes: "Transportation and Mobility Sections",
    mobilityCentersTypes: "Mobility Centers Types",
    suppliesNames: "Supplies and Goods Types",
    ...
    availabilityStatuses: "Operational Staff / Consumables Reality",
    infrastructureFacilities: "Infrastructure Facilities",
    administrativeServicesNames: "Administrative Services Types",
    otherServicesNames: "Administrative Services",
    facilityTypes: "Facility Types",
    participationTypes: "Participation Types",
    potentialPartners: "Potential Partners for Administrative Units",
  ...
  gridItems.forEach((item) => {
    ...
    if (modalId) {
      // Try to find the translation in the translations object first
        if (window.translations[lang][modalId]) {
    item.textContent = window.translations[lang][modalId];
      }
...
function toggleLanguage() {
  ...
  const langToggleText = document.getElementById("langToggleText");
  if (langToggleText) {
    langToggleText.textContent = window.currentLang === "ar" ? "Change Language" : "تغيير اللغة";
  }
  ...
  if (tableName === "needItems") {
    ...
    needItemsData.forEach((item, index) => {
      ...
      row.innerHTML = `
        ...
        <td class="text-center">${window.currentLang === "ar" ? item.itemNameArabic : item.itemName}</td>
        <td class="text-center">
          <button class="btn btn-link text-danger" onclick="deleteRow('needItems', ${index})" title="${window.currentLang === 'ar' ? 'حذف' : 'Delete'}">
            <i class="fas fa-trash"></i>
          </button>
          <button class="btn btn-link text-primary" onclick="editRow('needItems', ${index})" title="${window.currentLang === 'ar' ? 'تعديل' : 'Edit'}">
...
function deleteRow(tableName, index) {
  ...
  if (
    confirm(`هل أنت متأكد من حذف العنصر من الجدول ${tableName}؟`)
  ) {
    let data = [];
    switch (tableName) {
      case "needItems":
...
const tableHeadersTranslations = {
  ...
  potentialPartners: [
    ...
    { ar: " أسماء الشركاء المحتملين للوحدات الإدارية", en: "Potential Partner Name" },
    {
      ar: " أسماء الشركاء المحتملين للوحدات الإدارية باللغة العربية",
  ...
  insectControls: [
    { ar: "معرف أنشطة مكافحة الحشرات والقوارض", en: "Insect and Rodent Control Activities ID" },
    { ar: "اسم أنشطة مكافحة الحشرات والقوارض", en: "Insect and Rodent Control Activities Name" },
    {
      ar: "اسم أنشطة مكافحة الحشرات والقوارض باللغة العربية",
      en: "Insect and Rodent Control Activities Name (Arabic)",
    },
    {
      ar: "القيمة العمرانية المركبة",
      en: "Composite Urban Value",
...
function getCurrentDir() {
  return window.currentLang === "ar" ? "rtl" : "ltr";
}
...
function updateTableHeadersAndDir(tableName) {
  ...
  headers.forEach((header, idx) => {
    if (ths[idx]) ths[idx].textContent = header[window.currentLang] || header["ar"];
  });
}
...
function updateModalLanguage(modal) {
  ...
  if (modalContent) {
    modalContent.style.textAlign = window.currentLang === "ar" ? "right" : "left";
  }
  ...
  if (modalFooter) {
    modalFooter.style.flexDirection =
      window.currentLang === "ar" ? "row-reverse" : "row";
  }
  ...
  modal
    .querySelectorAll(".form-control, input, select, textarea")
    .forEach((input) => {
      input.style.textAlign = window.currentLang === "ar" ? "right" : "left";
      input.style.direction = window.currentLang === "ar" ? "rtl" : "ltr";
    });

  // Update labels within the modal
  modal.querySelectorAll(".form-label, label").forEach((label) => {
    label.style.textAlign = window.currentLang === "ar" ? "right" : "left";
    label.style.float = window.currentLang === "ar" ? "right" : "left";
...
function updateEditModalLang() {
  ...
  if (editModal) {
    ...
    if (title)
      title.textContent = window.currentLang === "ar" ? "تعديل البيانات" : "Edit Data";

    const cancelBtn = editModal.querySelector(".btn-secondary");
    if (cancelBtn)
      cancelBtn.textContent = window.currentLang === "ar" ? "إلغاء" : "Cancel";

    const saveBtn = editModal.querySelector("#saveEditBtn");
    if (saveBtn)
      saveBtn.textContent =
        window.currentLang === "ar" ? "حفظ التغييرات" : "Save Changes";
    ...
    editModal.querySelectorAll("label[for]").forEach((label) => {
      if (label.htmlFor === "editId")
        label.textContent = window.currentLang === "ar" ? "معرف:" : "ID:";
      if (label.htmlFor === "editName" && !label.textContent.includes("الجهة"))
        label.textContent =
          window.currentLang === "ar" ? "الاسم بالإنجليزية:" : "Name in English:";
      if (label.htmlFor === "editNameEn")
        label.textContent =
          window.currentLang === "ar" ? "الاسم بالإنجليزية:" : "Name in English:";
      if (label.htmlFor === "editNameAr")
        label.textContent =
          window.currentLang === "ar" ? "الاسم بالعربية:" : "Name in Arabic:";
      if (label.htmlFor === "editNameArabic")
        label.textContent =
          window.currentLang === "ar" ? "الاسم بالعربية:" : "Name in Arabic:";
      if (label.htmlFor === "editCode")
        label.textContent = window.currentLang === "ar" ? "الرمز:" : "Code:";
      if (label.htmlFor === "editCityId")
        label.textContent = window.currentLang === "ar" ? "معرف المدينة:" : "City ID:";
      if (label.htmlFor === "editDistributionDate")
        label.textContent =
          window.currentLang === "ar" ? "تاريخ التوزيع:" : "Distribution Date:";
      if (label.htmlFor === "editName" && label.textContent.includes("الجهة"))
        label.textContent =
          window.currentLang === "ar" ? "الجهة المتدخلة:" : "Intervening Party:";
...
displayData = function (tableName) {
  ...
  data.forEach((item, index) => {
    ...
    if (tableName === "needItems") {
      const displayName = window.currentLang === "ar" ? item.itemNameArabic : item.itemName;
      let row = `<tr>
        <td>${item.id}</td>
        <td>${displayName}</td>
        <td>
          <div class="btn-group">
            <button class="btn btn-edit square-action-btn" onclick="editRow('${tableName}', ${index})" title="${window.currentLang === 'ar' ? 'تعديل' : 'Edit'}">
              <i class="fas fa-edit"></i>
            </button>
            <button class="btn btn-danger square-action-btn" style="max-width: 50px;" onclick="deleteRow('${tableName}', ${index})" title="${window.currentLang === 'ar' ? 'حذف' : 'Delete'}">
